...
| Least privilege | Each pod can only communicate with exactly what it needs |
...
Kubernetes namespaces act as logical boundaries inside the cluster. By placing each tier in its own namespace, we can write Calico NetworkPolicies that say "only the backend namespace may talk to the database namespace." This is namespace-level microsegmentation which is a core pattern in Zero Trust network design. 
...
<pre>minikube start --driver=docker --cni=calico --cpus=2 --memory=3000 
</pre>
Flag explanations: 
...
On this version of Minikube, Calico pods land in _kube-system_ rather than _calico-system_. Both calico-kube-controllers and calico-node must show Running before proceeding.
...
Each namespace maps to one tier of the application. Calico NetworkPolicies reference these namespaces by their _kubernetes.io/metadata.name_ label.

---
### Deployment
Frontend — Nginx \
File: _k8s/deployments/nginx.yaml_ \
...
Key points: 
* Labels _app: nginx_ and _tier: frontend_ are used by Calico to identify this pod 
* NodePort service exposes it at _localhost:30080_ 
* All other pods use ClusterIP (internal only)

Backend — Flask API \
File: _k8s/deployments/flask.yaml_ \
...
<pre>kubectl apply -f k8s/deployments/flask.yaml
</pre>
Key points: 
...
Database — Postgres \
File: _k8s/deployments/postgres.yaml _ \
...
<pre>kubectl apply -f k8s/deployments/postgres.yaml
</pre>
Key points: 
...
Frontend Policy \
File: _k8s/network-policies/frontend-policy.yaml_
####shot

The DNS egress rule is critical as without it, Calico blocks DNS queries to CoreDNS (which lives in kube-system), and service name resolution fails entirely. This is a common issue when first implementing NetworkPolicies.

Backend Policy \
File: _k8s/network-policies/backend-policy.yaml_
##Shot
...
Database Policy \
File: _k8s/network-policies/database-policy.yaml_
yamlin ##shot
...
All tests run using _kubectl exec_ to jump inside pods and attempt connections.
...
<pre>kubectl exec -it nginx-pod -n frontend -- curl flask-service.backend.svc.cluster.local:5000
</pre>
Why it works: frontend-policy explicitly allows egress to backend on port 5000

Test 2 — Frontend → Database (Should be BLOCKED) 
<pre> kubectl exec -it nginx-pod -n frontend -- curl <postgres-clusterip>:5432 --max-time 10
</pre>
...
<pre>kubectl exec -it flask-pod -n backend -- python3 -c "
import socket
s = socket.socket()
s.settimeout(5)
...
CoreDNS (the cluster's internal DNS server) lives in the _kube-system_ namespace. When a pod tries to resolve a service name like_ flask-service.backend.svc.cluster.local_, it sends a DNS query to CoreDNS on port 53. If your NetworkPolicy doesn't explicitly allow egress to kube-system on port 53, Calico blocks those DNS queries and all hostname-based connections fail — even if the actual service connection would be allowed. Always include a DNS egress rule in your policies.

---
_Last updated: April 2026
Environment: Minikube v1.33.x · Kubernetes v1.35.1 · Calico (kube-system) · Ubuntu 24.04# k8s-security-lab_
